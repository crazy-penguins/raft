...
class CLIParsing:
    ...
    def leading_underscores_are_ignored(self):
        self._compare("--leading_underscore", "_leading_underscore", True)

    def trailing_underscores_are_ignored(self):
        self._compare("--trailing_underscore", "trailing_underscore_", True)

    def inverse_boolean_flags(self):
        self._compare("--no_true_bool", "true_bool", False)
    ...
    def list_type_flag_can_be_given_N_times_building_a_list(self):
        "my_task --my_list foo --my_list bar"
        # Test both the singular and plural cases, just to be safe.
        self._compare("--my_list foo", "my_list", ["foo"])
        self._compare("--my_list foo --my_list bar", "my_list", ["foo", "bar"])
